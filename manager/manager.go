...
// CreateTestUser is used to create a user object for testing
func (m *Manager) CreateTestUser(tesetEmail, testPassword, testAccountId string) (*model.User, error) {
	u := &model.User{
		Email:             tesetEmail,
		Password:          secret.New().HashPassword(testPassword),
		Active:            true,
		Verified:          true,
