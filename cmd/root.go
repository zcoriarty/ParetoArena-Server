...
import (
	...
	"github.com/zcoriarty/Backend/server"
...
var rootCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		var env string
		var ok bool
		if env, ok = os.LookupEnv("PARETO_ENV"); !ok {
			env = "dev"
...
func init() {
	...
	rootCmd.PersistentFlags().StringVar(&cfgFile, "config", "", "config file (default is $HOME/.pareto.yaml)")
...
func initConfig() {
	if cfgFile != "" {
		...
		// Search config in home directory with name ".pareto" (without extension).
		viper.AddConfigPath(home)
		viper.SetConfigName(".pareto")
	}
